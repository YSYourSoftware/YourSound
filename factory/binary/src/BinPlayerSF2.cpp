...
void action_import_sf2(const char *filepath) {
	try {
		...
		for (int i = 0; i < tsf_get_presetcount(sf2); i++) {
			...
			if (!file)
				throw std::runtime_error(
					"Could not open file: " +
					(library_location / (std::string(tsf_get_presetname(sf2, i)) + ".yspp")).string());
		...
		tsf_close(sf2);
	} catch (const std::exception &e) { ysbp_show_error(YourSound::get_error_source(), e.what()); }
}
...
BinPlayerSF2::~BinPlayerSF2() { tsf_close(m_soundfont); }

void BinPlayerSF2::note_on(const uint8_t midi_note_number, const float velocity) {
	tsf_channel_note_on(m_soundfont, 0, midi_note_number, velocity);
}
void BinPlayerSF2::note_off(const uint8_t midi_note_number) { tsf_channel_note_off(m_soundfont, 0, midi_note_number); }
void BinPlayerSF2::render(float *output_buffer, const uint16_t number_samples) {
...
void BinPlayerSF2::reset() {
	tsf_reset(m_soundfont);
	m_pitch_bend = 0.f;
}

void BinPlayerSF2::render_graphics() {
	if (ysbp_ui_begin_combo("Preset", tsf_get_presetname(m_soundfont, m_preset_id))) {
		for (uint16_t i = 0; i < tsf_get_presetcount(m_soundfont); i++) {

			const bool is_selected = m_preset_id == i;

			if (ysbp_ui_combo_selectable(tsf_get_presetname(m_soundfont, i), is_selected)) {
				m_preset_id = i;
	...
	ysbp_ui_begin_group();
	if (ysbp_ui_knob("Gain", &m_gain, -20.f, 10.f, 0.1f, "%.2fdB", ysbp_ui_ImGuiKnobVariant_Wiper, 75))
		tsf_set_output(m_soundfont, TSF_STEREO_INTERLEAVED, m_sample_rate, m_gain);
	ysbp_ui_end_group();
