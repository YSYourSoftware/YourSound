...
static void SDLCALL load_callback(void *userdata, const char *const *filelist, int filter) {
	...
	if (!input_file.read(reinterpret_cast<char *>(input_buffer), file_length)) {
		show_error("Failed to read input file");
		return;
	}
	...
	if (!bin_player) {
		show_error("Player pointer is a nullptr");
		return;
	}
...
int main(int argv, char *argc[]) {
	...
	SDL_Window *window = SDL_CreateWindow("YourSound Library Tester", static_cast<int>(1280 * main_scale),
										  static_cast<int>(950 * main_scale), window_flags);
	...
	while (run_main_loop) {
		...
		if (ImGui::BeginMenu("File")) {
			...
			if (ImGui::MenuItem("Load State"))
				SDL_ShowOpenFileDialog(load_callback, nullptr, window, yoursound_player_preset_filetype_filters, 2,
									   nullptr, false);
			ImGui::EndMenu();
		}
		...
		if (show_param_edit) {
			...
			if (ImGui::BeginCombo("##param_id", param_edit_id)) {
				...
				for (size_t i = 0; i < count; i++) { parameters[i] = new char[32]; }
