...
static void SDLCALL callback_install(void *userdata, const char *const *filelist, int filter) {
	...
	std::ifstream file{target_library_path};
	try {
		target_library = new SArc::SArchiveStream{file};
	} catch (std::exception &e) {
		show_error("Error reading SArc archive:\n" + std::string(e.what()));
		return;
	}
	...
	try {
		...
		show_error("Error reading YourSound library information:\n" + std::string(e.what()));
		return;
	}
	...
	try {
		...
		show_error("Malformed YourSound library information:\n" + std::string(e.what()));
		return;
	}
...
int main(int argv, char *argc[]) {
	...
	SDL_Window *window = SDL_CreateWindow("YourSound Library Manager", static_cast<int>(500 * main_scale),
										  static_cast<int>(650 * main_scale), window_flags);
	if (window == nullptr) {
		show_error(SDL_GetError());
		return 1;
	}
	...
	constexpr SDL_DialogFileFilter yoursound_filetype_filters[] = {{"YourSound Libraries", "yslib"},
																   {"All Files", "*"}};
	...
	while (run_main_loop) {
		...
		if (ImGui::Shortcut(ImGuiMod_Ctrl | ImGuiKey_I))
			SDL_ShowOpenFileDialog(callback_install, nullptr, window, yoursound_filetype_filters, 2, nullptr, false);

		if (ImGui::BeginMenuBar()) {
			if (ImGui::BeginMenu("File")) {
				if (ImGui::MenuItem("About")) display_about = true;
				if (ImGui::MenuItem("Install", "Ctrl+I", false, !display_install))
					SDL_ShowOpenFileDialog(callback_install, nullptr, window, yoursound_filetype_filters, 2, nullptr,
										   false);
				ImGui::EndMenu();
			}
