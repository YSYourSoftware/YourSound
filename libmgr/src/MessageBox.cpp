...
MBTrustPublisherResult ask_yesno_trust_publisher(const std::string &prompt, const std::string &message, const std::string &publisher, const std::string &title) {
	const std::wstring wide_prompt = to_wstring(prompt);
	const std::wstring wide_message = to_wstring(message);
	const std::wstring wide_title = to_wstring(title);
	const std::wstring wide_expand = to_wstring(std::format("Publisher: {}\nMalicious actors can impersonate other publishers.\nAlways check file hashes from the publisher's website.", publisher));
	...
	config.pszContent = wide_message.c_str();
	config.pszMainInstruction = wide_prompt.c_str();
	config.pszVerificationText = L"Trust Publisher";
	config.pszExpandedInformation = wide_expand.c_str();

	config.pszMainIcon = TD_SHIELD_ICON;

	int button_pressed = 0;
	BOOL verify_flag = 0;
	TaskDialogIndirect(&config, &button_pressed, nullptr, &verify_flag);

	if (button_pressed != 6) return NO;
	if (verify_flag == 1) return YES_AND_TRUST;
	return YES;
}
#else
...
MBTrustPublisherResult ask_yesno_trust_publisher(const std::string &prompt, const std::string &message, const std::string &title) {
	return static_cast<MBTrustPublisherResult>(yesno_handler(prompt, message, title, mb_warning_flags, mb_yesno_trust_buttons, 3));
}
#endif