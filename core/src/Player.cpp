...
PlayerInterface::PlayerInterface(YS_PlayerHandle player_handle, const std::filesystem::path &dll_path) :
	...
	YS_LOAD_API_FN(m_player_api, get_parameter);
...
void PlayerInterface::note_on(const uint8_t midi_note_number, const float velocity) {
	m_player_api.note_on(m_player_handle, midi_note_number, velocity);
}

void PlayerInterface::note_off(const uint8_t midi_note_number) {
	m_player_api.note_off(m_player_handle, midi_note_number);
}

void PlayerInterface::render(float *output_buffer, const uint16_t number_samples) {
	m_player_api.render(m_player_handle, output_buffer, number_samples);
}

uint64_t PlayerInterface::store_calc_size(const bool store_reference) {
	return m_player_api.store_calc_size(m_player_handle, store_reference);
}

void PlayerInterface::store(uint8_t *output_buffer, const bool store_reference) {
	m_player_api.store(m_player_handle, output_buffer, store_reference);
}
...
void PlayerInterface::set_parameter(const char *param_id, const float value) {
	m_player_api.set_parameter(m_player_handle, param_id, value);
}

float PlayerInterface::get_parameter(const char *param_id) {
	return m_player_api.get_parameter(m_player_handle, param_id);
}
...
void YourSound::free_binary_player(const PlayerInterface *binary_player) {
	...
	SARC_RUNTIME_ASSERT(destroy_bin_player, invalid_binary_player,
						"destroy_bin_player not found in library: " + binary_player->m_dll_path.string());
...
PlayerInterface *YourSound::load_binary_player(const std::filesystem::path &dll_path, const std::string &player_id) {
	...
	if (loaded_dlls.contains(dll_path)) {
		...
		SARC_RUNTIME_ASSERT(get_api_version, invalid_binary_player,
							"get_api_version not found in library: " + dll_path.string());
		SARC_RUNTIME_ASSERT(get_api_version() == YS_API_VERSION, SArc::version_mismatch,
							"Player API version mismatch in: " + dll_path.string() + " (expected " +
								std::to_string(YS_API_VERSION) + ", got " + std::to_string(get_api_version()) + ")");
	}

	const auto create_bin_player = reinterpret_cast<create_bin_player_fn>(GetProcAddress(dll, "create_bin_player"));
	SARC_RUNTIME_ASSERT(create_bin_player, invalid_binary_player,
						"create_bin_player not found in library: " + dll_path.string());

	YS_RackHandle bin_player = create_bin_player(player_id.c_str());
	SARC_RUNTIME_ASSERT(bin_player, player_not_found,
						"Player " + player_id + " not found in library: " + dll_path.string());
...
Player *YourSound::load_player_by_id(const std::string &player_id) {
	SARC_RUNTIME_ASSERT(BinPlayer::g_bp_registry.contains(player_id), player_not_found,
