...
void SampleResource::from_flac(const uint8_t *input_buffer, const size_t length) {
	...
	YS_RUNTIME_ASSERT(flac, std::runtime_error, "Failed to load FLAC data, check the FLAC file is valid.");

	YS_RUNTIME_ASSERT(flac->channels <= 2, std::runtime_error, "Too many channels in stream, max 2 supported.");
	...
	YS_RUNTIME_ASSERT(sample_count == flac->totalPCMFrameCount, std::runtime_error, "Samples read is not equal to total samples.");
...
void SampleResource::from_mp3(const uint8_t *input_buffer, const size_t length) {
	drmp3 mp3;
	YS_RUNTIME_ASSERT(drmp3_init_memory(&mp3, input_buffer, length, nullptr), std::runtime_error, "Failed to load MP3 data, check the MP3 file is valid.");

	YS_RUNTIME_ASSERT(mp3.channels <= 2, std::runtime_error, "Too many channels in stream, max 2 supported.");
	...
	YS_RUNTIME_ASSERT(sample_count == mp3.totalPCMFrameCount, std::runtime_error, "Samples read is not equal to total samples.");
...
void SampleResource::from_ogg(const uint8_t *input_buffer, const size_t length) {
	...
	YS_RUNTIME_ASSERT(vorbis_error == VORBIS__no_error, std::runtime_error, "Failed to load OGG Vorbis data, check the OGG file is valid.\n(error " + std::to_string(vorbis_error) + ")");

	const stb_vorbis_info ogg_info = stb_vorbis_get_info(ogg);
	YS_RUNTIME_ASSERT(ogg_info.channels <= 2, std::runtime_error, "Too many channels in stream, max 2 supported.");

	const uint32_t sample_count = stb_vorbis_stream_length_in_samples(ogg);

	auto *samples = new float[sample_count * 2];
	YS_RUNTIME_ASSERT(stb_vorbis_get_samples_float_interleaved(ogg, ogg_info.channels, ogg_info.channels == 2 ? samples : samples + sample_count, sample_count) == sample_count, std::runtime_error, "Samples read is not equal to total samples.");
...
void SampleResource::from_wav(const uint8_t *input_buffer, const size_t length) {
	drwav wav;
	YS_RUNTIME_ASSERT(drwav_init_memory(&wav, input_buffer, length, nullptr), std::runtime_error, "Failed to load WAV data, check the WAV file is valid.");

	YS_RUNTIME_ASSERT(wav.channels <= 2, std::runtime_error, "Too many channels in stream, max 2 supported.");
	...
	YS_RUNTIME_ASSERT(sample_count == wav.totalPCMFrameCount, std::runtime_error, "Samples read is not equal to total samples.");
